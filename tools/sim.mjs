...
const baseB = {
  ...
  // NEU: Markt-Sättigung. Verkauf bis sweet×marketSize → voller Preis; darüber Preis-Haircut.
  //   ratio = sold / marketSize ; satMult = ratio<=sweet ? 1 : max(floor, 1-(ratio-sweet)*steep)
  saturation: { enabled:false, sweet:0.85, steep:1.2, floor:0.55, pushMax:1.6 },
...
function premiumDemand(B, brew, S, sea, pm=1){
  ...
  const elas = clamp(1-(pm-1)*B.demand.elasticity, 0.4, 1.5);
...
function run(policy,B,trials=500){
  let bankrupt=0; const cashes=[]; let cap=0,mkt=0,tanks=0;
  for(let i=0;i<trials;i++){ const r=policy(B); if(r.bankrupt)bankrupt++; else {cashes.push(r.cash);cap+=r.capacity;mkt+=r.market;tanks+=r.extraTanks;} }
  cashes.sort((a,b)=>a-b); const n=cashes.length||1;
  return { bankrupt:(bankrupt/trials*100).toFixed(0)+"%", medCash:cashes[Math.floor(cashes.length/2)]||0,
...
let scenarios;
if(arg){ scenarios={ "OVERRIDE": deepMerge(clone(baseB), JSON.parse(arg)) }; }
else {
  scenarios={
    "Baseline (keine Sättigung)": clone(baseB),
    "Sättigung sweet.85 steep1.2 floor.55": deepMerge(clone(baseB),{saturation:{enabled:true,sweet:0.85,steep:1.2,floor:0.55}}),
    "Sättigung sweet.75 steep2.0 floor.45": deepMerge(clone(baseB),{saturation:{enabled:true,sweet:0.75,steep:2.0,floor:0.45}}),
    "Sättigung sweet.70 steep2.5 floor.40": deepMerge(clone(baseB),{saturation:{enabled:true,sweet:0.70,steep:2.5,floor:0.40}}),
  };
}
